...
model = torch.load('saves/quant_res50_model_ee.ptmodel').to(device)
print(model)
...
model.fc[0] = huffman_Linear(model.fc[0])
...
torch.save(model, f"saves/huffman_res50_model_ee.ptmodel")
torch.save(model.state_dict(), f"saves/huffman_res50_param_ee.ptmodel")
